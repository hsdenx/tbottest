...
if [ "$?" != "1" ];then
	echo "ERROR: Locking into Lab without passing lock id must fail"
	RET=1
fi
...
if [ "$?" != "0" ];then
	echo "ERROR: Locking into Lab with no lock must work"
	RET=1
fi
...
tbottest/newtbot_starter.py @tbotconfig/foo/args/argsfoo-noeth -f lablockid:foobar tbotconfig.ci.testlab.uname
if [ "$?" != "0" ];then
	echo "ERROR: Locking into Lab with lock foobar must work"
	RET=1
fi
...
if [ "$?" != "1" ];then
	echo "ERROR: Locking into Lab with wrong lock foobar2 must fail"
	RET=1
fi
...
if [ "$?" != "0" ];then
	echo "ERROR: remove Locking with correct lock foobar must work"
	RET=1
fi
